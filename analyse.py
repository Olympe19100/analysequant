...
class CointegrationStrategies:
    ...
    def identify_cointegrated_pairs(self):
        """
        Identifie les paires co-intégrées dans l'ensemble de données.
        """
        st.write("### Identification des paires co-intégrées 🔍")
        for i in range(len(self.data.columns)):
            for j in range(i + 1, len(self.data.columns)):
                ...
                if pvalue < 0.05:
                    ...
                    st.write(f"**Paire co-intégrée : {asset1} et {asset2} (p-value={pvalue:.4f})**")

    def calculate_hedge_ratios(self):
        """
        Calcule le ratio de couverture optimal pour chaque paire co-intégrée.
        """
        st.write("### Calcul des ratios de couverture pour chaque paire 📊")
    ...
    def calculate_spreads(self):
        """
        Calcule le spread pour chaque paire co-intégrée en utilisant le ratio de couverture.
        """
        st.write("### Calcul des spreads 📉")
    ...
    def generate_signals(self):
        """
        Génère des signaux de trading basés sur le z-score des spreads.
        """
        ...
        for (asset1, asset2), spread in self.spreads.items():
            ...
            st.write(f"**Signaux pour {asset1}/{asset2} :**")
            st.write(f"Nombre de signaux d'achat : {buy_signal.sum()}, Nombre de signaux de vente : {sell_signal.sum()}")

            # Calcul de la taille de position en fonction de l'Edge Ratio (exemple simplifié)
            edge_ratio = (sell_signal.sum() - buy_signal.sum()) / (sell_signal.sum() + buy_signal.sum())
            position_size = edge_ratio * 100
            st.write(f"Taille de position suggérée (en % du capital) pour la paire {asset1}/{asset2} : {position_size:.2f}%")
...
class ComprehensiveCryptoCommoAnalyzer:
    def __init__(self, tickers, names, start_date):
        ...
        self.cointegration = None  # Pour stocker les résultats de la co-intégration
...
def main():
    ...
    st.header('4. Stratégies de Co-Intégration')
