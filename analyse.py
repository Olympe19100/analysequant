...
class ComprehensiveCryptoCommoAnalyzer:
    ...
    def plot_significant_relationships(self, investment_amount):
        ...
        for col in significant_assets:
            ...
            if position_size > 0:  # Acheter du Bitcoin, vendre l'autre crypto
                btc_units = (investment_amount * position_size / 100) / btc_price
                other_units = (investment_amount * (100 - position_size) / 100) / other_price
                action_btc = "Acheter"
                action_other = "Vendre"
            else:  # Vendre du Bitcoin, acheter l'autre crypto
                btc_units = (investment_amount * abs(position_size) / 100) / btc_price
                other_units = (investment_amount * (100 - abs(position_size)) / 100) / other_price
                action_btc = "Vendre"
                ...
                st.markdown(f"<div class='info-box'><h3>Stratégie pour la paire Bitcoin - {self.names[self.tickers.index(col)]} :</h3>"
                        ...
                        f"Pour un investissement de {investment_amount:.2f}$ :<br>"
                        f"- {action_btc} {btc_units:.4f} unités de Bitcoin (BTC)<br>"
                        f"- {action_other} {other_units:.4f} unités de {self.names[self.tickers.index(col)]}<br>"
                        f"Prix actuel de BTC : {btc_price:.2f}$<br>"
                        f"Prix actuel de {self.names[self.tickers.index(col)]} : {other_price:.2f}$"
                        "</div>", unsafe_allow_html=True)
...
def main():
    ...
    if page == "Accueil":
        ...
        st.info("👈 Utilisez le menu à gauche pour commencer l'analyse et entrer votre montant d'investissement.")
        ...
        <div class="explanation">
            ...
            Nous calculons également le nombre d'unités à acheter ou à vendre pour chaque cryptomonnaie basé sur votre montant d'investissement de {investment_amount:.2f}$.</p>
        </div>
        """.format(investment_amount=investment_amount), unsafe_allow_html=True)
        
        analyzer.random_forest_model()
        analyzer.plot_significant_relationships(investment_amount)
        ...
        <div class="info-box">
            ...
            <ul>
                ...
                <li>Le nombre d'unités à acheter ou à vendre est calculé en fonction de la taille de position suggérée et des prix actuels, basé sur votre montant d'investissement.</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
